...
stats_evalues_virtual <- function(stats = c("median", "range"), virtualsp_folder,
                                  virtualsp_format, var_folder, var_format,
  ...
  cat("Preparing statistics from environmental layers:\n")
  n_vars <- raster::nlayers(variables)
  var_stats <- lapply(1:n_vars, function(i) {
    sp_stats <- lapply(1:length(mlist), function(j) {
      ## data
      if (virtualsp_format %in% c("shp", "gpkg")) {
        if (virtualsp_format == "shp") {
          M <- rgdal::readOGR(dsn = virtualsp_folder, layer = mlist[j],
                              verbose = FALSE)
        } else {
          M <- rgdal::readOGR(paste0(virtualsp_folder, "/",
      ...
      return(unlist(m_stats))
    })

    # preparing tables with results
    m_table <- data.frame(Species = mlist, do.call(rbind, sp_stats))
